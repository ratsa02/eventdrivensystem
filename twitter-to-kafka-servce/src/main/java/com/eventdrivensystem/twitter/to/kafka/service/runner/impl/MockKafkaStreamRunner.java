...
public class MockKafkaStreamRunner implements StreamRunner {
    ...
    public void start() throws TwitterException {
        ...
        long sleepTimsInMs = twitterToKafkaServiceConfigData.getMockSleeps();
        LOG.info("Staring mock filtering tweeter for keywords {}",Arrays.toString(keywords));

        simulateTwitterStream(keywords, minTweetLength, maxTweetLength, sleepTimsInMs);

    }

    private void simulateTwitterStream(String[] keywords, int minTweetLength, int maxTweetLength, long sleepTimsInMs) throws TwitterException {
    ...
    private void sleep(long sleepTimsInMs) {
        try{
            Thread.sleep(sleepTimsInMs);
        }catch(InterruptedException e){
            throw new TwitterToKafkaServiceException("Error while sleeping on waiting for new status to create !!!");
        }
    }

    private String getFormattedTweet(String[] keywords, int minTweetLength, int maxTweetLength) {
        String[] params = new String[]{
                ZonedDateTime.now().format(DateTimeFormatter.ofPattern(TWITTER_STATUS_DATE_FORMAT)),String.valueOf(ThreadLocalRandom.current().nextLong(Long.MAX_VALUE)),
